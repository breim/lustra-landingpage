...
const HARNESS_MONOGRAM: Record<string, string> = {
  'Claude Code': 'Cc',
  Cursor: 'Cr',
  'Gemini CLI': 'Gm',
  'Codex CLI': 'Cx',
...
export function HarnessMonogram({ name }: { name: string }) {
  const mark = HARNESS_MONOGRAM[name] ?? name.slice(0, 2)

  return (
    <span
      aria-hidden="true"
      className="w-6 shrink-0 text-center font-display text-[0.7rem] font-extrabold uppercase tracking-tight opacity-45"
    >
      {mark}
    </span>
  )
